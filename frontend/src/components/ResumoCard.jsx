...
export default function ResumoCard({ titulo, valor, subtitulo, corIcone, icon }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200/80 p-3 min-h-[5.25rem] flex items-start justify-between gap-2 sm:min-h-24 sm:p-4 sm:gap-4">
      <div className="min-w-0">
        <p className="text-[11px] font-medium text-[#4F5D75] leading-tight sm:text-xs">{titulo}</p>
        <p className="text-lg font-bold text-[#0B1437] mt-1 leading-tight break-words sm:text-2xl">{valor}</p>
        {subtitulo && (
      ...
      <div
        className={`h-8 w-8 shrink-0 rounded-full flex items-center justify-center sm:h-10 sm:w-10 ${iconTone}`}
      >
